...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function mysum(val, name, record) {
            return "<div id='div" + record["TaskID"] + "'><%=StrSrc("Total")%>:" + "</div>";
        }
    </script>
    <style type="text/css">
...
<body onload="setTimeout('parent.CloseLoading();',10);">
    ...
    <div id="dEditWinTaskItemBox" class="EditWinTaskItemBox">
        <div class="JumpWiX" style="padding-right: 3px; padding-top: 2px;">
            <img alt="<%=StrSrc("Close")%>" src="../../img/X.png" class="imngX" onclick="CloseEditWinTaskItemBox();" />
        </div>
        ...
        <table cellpadding="4" cellspacing="2">
            <tr>
                ...
                <td class="EditForm item" style="vertical-align: top;">
                   <%=StrSrc("Responsibility")%>:
                </td>
                <td class="EditForm val">
                    <table cellpadding="0" cellspacing="0">
                        <tr>
                            <td>
                                <a id="aPop" href="#" onclick="ShowCtlPopulations();" style="color: Blue;"><div style="color:Black;text-decoration:'none' !important;"><%=StrSrc("Loading")%>...<img alt="<%=StrSrc("Loading")%>..." src="../../img/loading1.gif" width="16px"/></div></a>
                            </td>
                        </tr>
                    </table>
                </td>
        ...
        <div id="div3" style="text-align: center; float: none; padding-top: 10px;">
            <input type="button" value="<%=StrSrc("Save")%>" class="MSBtnGeneral" style="background-image: url('../../Img/ok.png');
    ...
    function SetTaskData(isToDelete) {
            if( $('#aPop').text()=="<%=StrSrc("Populations")%>" || ($('#<%=ctlPopulationsR.hdnParentsPopulationID %>').val() =="" && $('#<%=ctlPopulationsR.hdnItemsPopulationID %>').val() ==""))  
            {
            ...
            {
                alert("<%=StrSrc("SelectTopic")%>");
                return;
            }

            if($('#<%=txtFromDate.ClientID %>').val()=="")
            {
                alert("<%=StrSrc("SelectFDate")%>");
                return;
            }

            if($('#<%=txtToDate.ClientID %>').val()=="")
            {
                alert("<%=StrSrc("SelectTDate")%>");
                return;
            }

            if($('#<%=txtAlarmDate.ClientID %>').val()=="")
            {
                alert("<%=StrSrc("SelectDisplayTime")%>");
                return;
            }
            var ConditionID="2";
   ...
   function ShowCtlPopulations() {
    ...
    function SetGrid() {
            ...
            $("#jQGrid").jqGrid({
                ...
                colNames: ['#','<%=StrSrc("Task")%>', '<%=StrSrc("Type")%>','<%=StrSrc("Code")%>', '<%=StrSrc("Name")%>','<%=StrSrc("Remark")%>','<%=StrSrc("AttachedFiles")%>','<%=StrSrc("StatusDate")%>','<%=StrSrc("Status")%>','*','TopicID','SubTopic','ReportUser','ClassificationID','PriorityID','dtReport','dtTaskEnd','ConditionID','TaskStatusID','dtStatus','DateFrom','DateTo','AlarmDate','Condition','Name'],
                ...
                loadComplete: function (data) {
                    $('.ui-pg-input')[0].parentElement.childNodes[0].data = " <%=StrSrc("of")%> ";
                    $('.ui-pg-input')[0].parentElement.childNodes[2].data = " <%=StrSrc("Page")%> ";
...
//                    }
                    ...
                    for (var i = 0; i < localData.length; i++) {
                        ...
                        if(localData[i]["TaskID"] != OldTadkID)
                        {
                            ...
                            $('#div'+OldTadkID).text("<%=StrSrc("Total")%>: "+counter.toString());
                            OldTadkID = localData[i]["TaskID"];
                            counter=0;
                        }
                        counter++;
