    ...
    $.fn.jqGrid = function (pin) {
		...
		setPager = function (pgid, tp) {
		    ...
		    if (ts.p.pgbuttons === true) {
				...
				fp = true, pp = true, np = true, lp = true;
		            ...
		            if (selclick) {
		                populate();
		                refreshHight();//Sets the grid size to window size
		            }
		...
		getColumnHeaderIndex = function (th) {
            ...
            if (withSearch) {
                ...
                thead2 += "<tr class='ui-jqgrid-txt' style='display:none;' role='rowheader'>";
                for (i = 0; i < this.p.colNames.length - 3; i++) {
                    thead2 += "<th style='height:24px;'  id='" + ts.p.id + "_" + ts.p.colModel[i].name + ">";
                    thead2 += "<div id='jqgh_" + ts.p.id + "_" + ts.p.colModel[i].name.toLowerCase() + "' " + tdc + " class='divSearchContralByCol_" + ts.p.colModel[i].name.toLowerCase() + "'>";
                    thead2 += " <input type='text' style='display:none;' id='Search" + ts.p.colModel[i].name + "' class='txtHead SearchOnTop' onkeyup='SearchBytxt(this)'; >";
                    thead2 += "</div></th>";
                }
                thead2 += "</tr>";
            }
            ...
            if (ts.p.caption) {
                ...
                if (ts.p.hidegrid === true) {
					...
					counter, self = this;
                        ...
                        //            if (onHdCl) {
                        //                if (!hg) { ts.p.onHeaderClick.call(ts, ts.p.gridstate, e); }
                        //            }
                        //        }
                        //    });
        ...
        navGrid: function (elem, o, pEdit, pAdd, pDel, pSearch, pView) {
            ...
            return this.each(function () {
                ...
                for (i = 0; i < clone; i++) {
                    ...
                    //if (o.search) { ביטול החיפוש והרענון
                    //    tbd = $("<td class='ui-pg-button ui-corner-all'></td>");
                    //    pSearch = pSearch || {};
