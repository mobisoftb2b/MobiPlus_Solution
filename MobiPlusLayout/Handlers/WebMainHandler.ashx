...
public class MainHandler : HandlerBaseCls
{
    ...
    private void GetCustomersForAgent(HttpContext context, string FromAgentID, string RouteDate)
    {
        ...
        string url = "http://" + System.Configuration.ConfigurationManager.AppSettings["ServerAddress"].ToString() + "/MobiWebServices/MPLayoutService.asmx/GetCustomersForAgent?AgentID=" + FromAgentID +"&RouteDate=" + RouteDate + 
    ...
    {
        WebClient client = new WebClient();
        string url = "http://" + System.Configuration.ConfigurationManager.AppSettings["ServerAddress"].ToString() + "/MobiWebServices/MPLayoutService.asmx/GetAllCustomersForDates?" +
            "ConString=" + ConStrings.DicAllConStrings[SessionProjectName];
        string str = client.DownloadString(url);
        ResponseJSON(context, context.Server.UrlDecode(str));
    }
    ...
    {
        WebClient client = new WebClient();
        string url = "http://" + System.Configuration.ConfigurationManager.AppSettings["ServerAddress"].ToString() + "/MobiWebServices/MPLayoutService.asmx/GetNumerators" +
            "?ConString=" + ConStrings.DicAllConStrings[SessionProjectName];
        string str = client.DownloadString(url);
    ...
    {
        WebClient client = new WebClient();
        string str = client.DownloadString("http://" + System.Configuration.ConfigurationManager.AppSettings["ServerAddress"].ToString() + "/MobiWebServices/MPLayoutService.asmx/GetNumeratorsForAgent?AgentID=" + AgentID +
            "&ConString=" + ConStrings.DicAllConStrings[SessionProjectName]);
