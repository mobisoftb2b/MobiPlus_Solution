...
class TasksGrid extends Component{
    ...
    dateFormatter(cell, row){
        if (cell)
            return cell.toString().substr(6, 2) + "/" + cell.toString().substr(4, 2) + "/" + cell.toString().substr(0, 4);
        else
            return cell; 
    }
                        ...
                        <Col lg={ 12 }>
                            <Panel>
                                <BootstrapTable data={data} search options={ options } insertRow deleteRow={true} selectRow={selectRowProp} striped hover condensed>
                                    <TableHeaderColumn dataField="TaskID" thStyle={{'verticalAlign':'top'}} isKey={true}  headerAlign='center' dataAlign='center' width='40' dataSort={true}>{col.TaskID}</TableHeaderColumn>
                                    <TableHeaderColumn dataField="TaskTypeDesc" thStyle={{'verticalAlign':'top'}} width='100' editable={{type: 'textarea'}} dataSort={true} dataAlign={recordpos} >{col.TaskTypeDesc}</TableHeaderColumn>
                                    <TableHeaderColumn dataField="TaskDesc" thStyle={{'verticalAlign':'top'}} width='100' editable={{ type: 'textarea' }} dataSort={true} dataAlign={recordpos} >{col.TaskDesc}</TableHeaderColumn>                                                                         
                                    <TableHeaderColumn dataField="UsersTasksName"  thStyle={{'verticalAlign':'top'}} width='100' dataSort={true} dataAlign={recordpos} >{col.TaskUser}</TableHeaderColumn>
                                    <TableHeaderColumn dataField="CustomerCode"  thStyle={{'verticalAlign':'top'}}  width='100' dataSort={true} dataAlign={recordpos} >{col.CustomerCode}</TableHeaderColumn>
                                    <TableHeaderColumn dataField="DateTo" thStyle={{'verticalAlign':'top'}} width='100' editable={{ type: 'textarea' }} dataAlign='center'  dataFormat={this.dateFormatter} dataSort={true}>{col.TaskEdit_ToDate}</TableHeaderColumn>
                                    <TableHeaderColumn dataField="CustAddress" thStyle={{'verticalAlign':'top'}} width='100' editable={{ type: 'textarea' }} dataAlign={recordpos}  dataSort={true}>{col.CustAddress}</TableHeaderColumn>
                                    <TableHeaderColumn dataField="CustCity" thStyle={{'verticalAlign':'top'}} width='100' dataSort={true} dataAlign={recordpos} >{col.CustCity}</TableHeaderColumn>  
                                </BootstrapTable>
