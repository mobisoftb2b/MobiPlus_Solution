...
class DashboardContainer extends Component {
    ...
    componentDidMount() {         
        ...
        this.timer = setInterval(() => {
            this.props.dispatch(changeFilter(this.props.filter));
        }, 120000);
    }
    ...
    renderFilter() {
        return (<div><FilterDropdown HiddenColName noCaret initFilterData={this.props.filterData} /></div>);
        }
                ...
                <ContentWrapper>
                    <Tabs activeKey={this.state.key} onSelect={this.handleSelect.bind(this)} justified id="tabID">
                        <Tab eventKey={1} title={local.dashboard.tabs.MAIN}>
                          ...
                          <Tab eventKey={2} title={local.dashboard.tabs.CUSTOMERS}>
                            <Row>
                                <Col lg={1}>{this.renderFilter()}</Col>
                            </Row>
                            <Row >
                                <Col md={12} >{this.renderCustomersGrid()}</Col>
                            </Row>
                        </Tab>
                        ...
                        <Tab eventKey={4} title={local.dashboard.tabs.MAP}>
                            <Row>
                        ...
                        <Tab eventKey={5} title={local.dashboard.tabs.ENDDAY}>
                           <Row>
                                <Col lg={1}>{this.renderFilter()}</Col>
                                 ...
                                 <Tab eventKey={6} title={local.dashboard.tabs.COLLECTIONNOTES}>
                                   <Row>
                                        <Col lg={1}>{this.renderFilter()}</Col>
                                    </Row>
                                    <Row >
                                ...
                                <Tab eventKey={7} title={local.dashboard.tabs.DRIVERREPORT}>
                                   <Row>
                                        <Col lg={1}>{this.renderFilter()}</Col>
                                    </Row>
                                    <Row >
