...
class ToDatePicker extends Component {
    constructor(props) {
        super(props);
        let cDate = this.props.filter.ToDate == null ? moment()._d : moment(this.props.filter.ToDate)._d;
        this.state = {
            currDate: cDate
        };
    ...
    ChangeCurrDateHandler = (event, date) => {
        ...
        filter.ToDate = moment(date).format('YYYYMMDD') || moment()._d;
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps.filter !== state) {
            let cDate = nextProps.filter.ToDate == null ? moment()._d : moment(nextProps.filter.ToDate)._d
            this.setState({currDate: cDate});
        }
    }
    ...
    render() {
        ...
        return (
            <div style={{'marginTop': '-22px'}}>
                <MuiThemeProvider>
                    <DatePicker hintText={this.props.local.filter.TODATE}
...
export default connect(mapStateToProps)(ToDatePicker);
