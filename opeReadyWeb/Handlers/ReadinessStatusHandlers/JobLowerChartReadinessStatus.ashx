<%@ WebHandler Class="JobLowerChartReadinessStatus" Language="C#" %>
...
public class JobLowerChartReadinessStatus : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    public void ProcessRequest(HttpContext context)
    {
        ...
        try
        {
            ...
            {
                result = _readiness.JobReadiness_Select(unitID, jobID, HttpContext.Current.Session["LanguageID"] == null ? 1 : int.Parse(HttpContext.Current.Session["LanguageID"].ToString()))
                    .Where(i => i.JobReadiness_Date <= DateTime.Now && i.JobReadiness_Date >= DateTime.Now.AddMonths(-24)).ToList(); 
            }
            if (result.Count > 0)
            {
                using (RuntimeChart chartCol = new RuntimeChart())
                {
                    ...
                    foreach (JobReadiness jobRead in result.OrderBy(x => x.JobReadiness_Date))
                    {
                        series.Points.AddY(jobRead.JobReadiness_Score);
                        series.Points[count].AxisLabel = jobRead.JobReadiness_Date.ToString("y", new System.Globalization.CultureInfo("en-US"));
                        count++;
                    }
                    chartCol.SeriesPoints = series;
