/// <reference path="jquery.color.utils-0.1.0.js" />
...
jQuery.fn.colourPicker = function (conf) {
	...
	return this.each(function () {
		...
		var icon = jQuery('<a href="#"><img style="border:0;padding:5px; vertical-align:middle" src="' + config.ico + '" alt="' + config.openTxt + '" /></a>').insertAfter(select);
		var input = jQuery(config.inputElement).insertAfter(select);
		var loc = '';

		var bgcolor = colourNameToHex(input.val());
		...
		//                        input.css({ background: bgcolor, color: '#' + WhiteBlackInvert(input.val()) });
		//                    });
