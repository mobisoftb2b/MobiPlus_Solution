...
BiBrowserCheck.constants = {
    OPACITY_STYLE: BiBrowserCheck.moz ? "MozOpacity" : BiBrowserCheck.webkit ? "WebkitOpacity" : "",
...
_p._stringBundle.addBundle("en", {
    ...
    ApplicationErrorInMain: "Application error: %1"
});
...
function BiComponent(sName) {
    ...
    this._htmlProperties = {
        id: this._className + (BiComponent.STRING_DASH + (++BiComponent._componentCount)),
...
window.bindows = {
    loadGaugeIntoDiv: function (sUri, divId) {
        ...
        for (var i = 0; i < ids.length; i++) g[ids[i]] = gauge.getComponentById(ids[i]);
        return g;
    },
    loadXMLGaugeIntoDiv: function (sXML, divId) {
        if (!application._window) {
            application._adfPath = new BiUri(application._uri, "./");
            application._window = new BiApplicationWindow;
            application._window._onkeyevent = BiAccessType.FUNCTION_EMPTY;
        ...
        var gaugeClass = BiXmlResourceParser.getClassFromDocument(sXML);
        var gauge = new gaugeClass();
        gauge.setVisible(false);
        var c = new BiInlineComponent(divId);
        c.add(gauge);
...
(function () {
    ...
    var packagedName = "jquery.gauges.js";
