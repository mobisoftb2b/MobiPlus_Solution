...
function ShowAddFormMD() {
    ...
    for (var i = 0; i < arr.length; i++) {
        if ($("#txt" + arr[i] + i).length > 0) {
            $("#txt" + arr[i] + i).val("");
            $("#txt" + arr[i] + i).attr('ReadOnly', false);
            if (i == 0) {
                onFocus = "#txt" + arr[i] + i;
            }
            boxHeight += 42;
            ...
            for (var i = 0; i < arr.length; i++) {
                for (var j = 0; j < arrPrimary.length; j++) {
                    if (arr[i] == arrPrimary[j].toLowerCase()) {
                        if (firstUnique) {
                            firstUnique = false;
                            Unique += arrPrimary[j] + ";" + Row[arrUpCase[i]];
                        } else {
                            Unique += ";" + arrPrimary[j] + ";" + Row[arrUpCase[i]];
                        }
                    }
            ...
            for (var i = 0; i < arr.length; i++) {
                if (i == 0) {
                    switch (strHdnn[i]) {
                        ...
                        case "Dateyyyymmdd":
                        case "DateTime":
                            Prams += arr[i] + ";" + escape($("#txt" + arr[i] + i)[0].value);
                            break;
                        case "boolean":
                            var bool = 0;
                            if ($("#cxx" + arr[i] + i)[0].checked) {
                                bool = 1;
                            }
                            Prams += arr[i] + ";" + bool;
                            break;
                        case "LOV":
                            Prams += arr[i] + ";" + escape($("#DDL" + arr[i] + i)[0].value);
                            break;
                    }
...
function initwData(data) {
    ...
    $("#gview_" + GridID)[0].children[0].children[0].onclick = HideSearchRow;
...
function RefreshMDBuURL(url) {
    ...
    pagePosition = $('#jQGrid').getGridParam('page');
    $("#jQGrid").trigger("reloadGrid");
    //$('#jQGrid').setGridParam({ page: 1, datatype: "json", url: url }).trigger('reloadGrid');
}

function RefreshMDBuData(data) {
    $('#jQGrid').setGridParam({ page: 1, datatype: "jsonstring", datastr: data }).trigger('reloadGrid');
}
...
function SearchBytxt(obj) {
    ...
    for (var i = 0; i < $('.SearchOnTop').length; i++) {
        if ($('.SearchOnTop')[i].id != "") {
            if (i == 0) {
                ColName += $('.SearchOnTop')[i].id.substr(6, obj.id.length);
