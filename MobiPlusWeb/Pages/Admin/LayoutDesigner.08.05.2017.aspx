<%@ Page Title="עיצוב מסכים" Language="C#" MasterPageFile="~/MasterPages/MainMasterPage.master"
    AutoEventWireup="true" CodeFile="LayoutDesigner.08.05.2017.aspx.cs" Inherits="Pages_Admin_LayoutDesigner" ValidateRequest="false" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function CheckForAddFragment() {
            SelID++;
            alert(IndexForGrafReal.toString());
            //if (isDragging) {
            var iDiv = document.createElement('div');
            ...
            else if (IndexForGrafReal.toString() == "2" || isGraf) {
            ...
            <tr>
                <td style="vertical-align: top; text-align: right;">בחר פרויקט:
                    ...
                    <asp:UpdatePanel runat="server" ID="UpdatePanelddlForms" UpdateMode="Conditional">
                        <ContentTemplate>
                            <asp:DropDownList runat="server" ID="ddlForms" Width="200px" ClientIDMode="Static" size="20">
                            </asp:DropDownList>
                        </ContentTemplate>
                    </asp:UpdatePanel>
                    <br />
                ...
                <td>
                    ...
                    <div id="divForm" class="divForm" style="visibility: hidden;">
                        <div id="tabs" style="text-align: right; direction: rtl; width: 99.5%; height: 99.5%"
                            ...
                            <div class="sortable" style="height: 100%;">
                            </div>
                        </div>
    ...
    function onTabClick(tabID, index) {
        ...
        function SaveData() {
            ...
            request.fail(function (jqXHR, textStatus, errorThrown) {
                if (jqXHR.status == 200) {
                    ...
                    //alert(arrOuterData.length);
                    //alert($('#tabs')[0].scrollHeight);

                    for (var i = 0; i < arrOuterData.length; i++)//tabs
                    {
                        if (arrTabIDsTouch[i]) {//&& lastarrOuterData != arrOuterData[i]
                            ...
                            if (div.childNodes[0].children && arrOuterData[i] && i>0 ) {
                                ...
                                for (var j = 0; j < div.childNodes[0].children.length; j++)//widgets
                                {
                                    if (div.childNodes[0].children[j].id.toString().indexOf('id_') > -1) {
                                        if ($('#' + div.childNodes[0].children[j].id).css("display") == "block") {
                                            if (minTop > $('#' + div.childNodes[0].children[j].id).offset().top)
                                                minTop = $('#' + div.childNodes[0].children[j].id).offset().top;
                                        }
                                    }
                                }
                                ...
                                for (var j = 0; j < div.childNodes[0].children.length; j++)//widgets
                                {
                                    if (div.childNodes[0].children[j].id.toString().indexOf('id_') > -1) {
                                        if ($('#' + div.childNodes[0].children[j].id).css("display") == "block") {
                                            ...
                                            strJson += "\"TabId\": " + i + ",";
                                            strJson += "\"Top\": \"" + ($('#' + div.childNodes[0].children[j].id).offset().top * 1.0 + 1.0 * 10.0) + "\",";
                                            ...
                                            strJson += "\"HeightWeight\": " + formatMoney(height * 1.0 * 100 * 1.0 / h * 1.0, 2) + ",";
                                            strJson += "\"TopWeight\": " + formatMoney((($('#' + div.childNodes[0].children[j].id).offset().top) - minTop) * 1.0 * 100 * 1.0 / h * 1.0, 2) + ",";
                                            strJson += "\"LeftWeight\": " + formatMoney(left * 1.0 * 100 * 1.0 / ParentWidth * 1.0, 2) + ",";
                                            strJson += "},";
        ...
        function LayoutManager_CreateXML(tabID) {
            if (tabID) {
                ...
                $.get("../../Handlers/MainHandler.ashx?MethodName=LayoutManager_CreateXML&TabID=" + tabID + "&Tiks=<%= DateTime.Now.Ticks.ToString()%>",
                function() {
                ...
                <%--request = $.ajax({
                    ...
                    request.done(function (response, textStatus, jqXHR) {
                        if (jqXHR.status == 200 && jqXHR.responseText == "ret: True") {
                            ...
                            console.log("אראה שגיאה בשמירת הXML");
                            //alert("אראה שגיאה בשמירת הXML");
                            //alert("Error");
                        }
                    });
            ...
            function LoadFormData() {
                ...
                for (var i = 0; i < response.length; i++) {
                    ...
                    if (response[i].TabHTMLLayout != "") {
                        ...
                        arrOuterData[response[i]] = ddiv.outerHTML;

                        //    arrTabIDs.push(response[i].TabID);
                        //  arrTabIDs[response[i]] = response[i].TabID;
                ...
                //arrTabIDsTouch[response[0]] = true;
                arrTabIDsTouch.push(true);
            });
            request.fail(function (jqXHR, textStatus, errorThrown) {
                if (jqXHR.status == 200) {
